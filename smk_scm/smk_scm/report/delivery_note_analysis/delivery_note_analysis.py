...
def execute(filters=None):
	...
	data = prepare_data(data, so_elapsed_time, filters)
...
def get_data(conditions, filters):
	data = frappe.db.sql(
		...
		SELECT
			...
			so.status, so.per_delivered, so.customer, soi.item_code,
		...
		LEFT JOIN `tabWork Order` wo
			ON so.name = wo.sales_order AND wo.docstatus = 1 AND wo.production_item = soi.item_code
		WHERE
...
def get_columns(filters):
	columns = [
		...
		{"label": _("% Delivered"), "fieldname": "per_delivered", "fieldtype": "Data", "width": 100},
		{
			"label": _("Customer"),
			"fieldname": "customer",
			"fieldtype": "Link",
