		...
		for rfq in rfq_list:
			...
			for item in doc.items:
				sqi_count = frappe.db.sql(
					...
					FROM
						`tabSQ for New Item Item` as sqi,
						`tabSQ for New Item` as sq
...
def make_purchase_invoice(source_name, target_doc=None):
	doc = get_mapped_doc(
		...
		source_name,
		{
			...
			"SQ for New Item Item": {"doctype": "Purchase Invoice Item"},
...
def set_expired_status():
	frappe.db.sql(
		"""
		UPDATE
			`tabSQ for New Item` SET `status` = 'Expired'
		WHERE
